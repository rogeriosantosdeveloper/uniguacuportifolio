...
// No futuro, você pode mover isso para um arquivo de constantes compartilhado
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('http://localhost:8080/api/auth/register', {
        ...
        // Enviamos apenas os dados que o backend espera
        body: JSON.stringify({
      ...
      setMessage('Usuário registrado com sucesso! Você será redirecionado para o login em 3 segundos.');
      setTimeout(() => {
        router.push('/login');
      }, 3000);
  ...
  return (
    <main className="flex justify-center items-center py-12">
        <div className="w-full max-w-md bg-white p-8 rounded-lg shadow-md">
            <h1 className="text-2xl font-bold text-center text-uniguacu-blue mb-6">Criar Conta</h1>
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  ...
                  <input type="text" id="nomeCompleto" name="nomeCompleto" value={formData.nomeCompleto} onChange={handleChange} className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700" required />
                </div>
                ...
                <div>
                    ...
                    <select id="curso" name="curso" value={formData.curso} onChange={handleChange} className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 bg-white" required>
                ...
                <div>
                    ...
                    <select id="turno" name="turno" value={formData.turno} onChange={handleChange} className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 bg-white" required>
                ...
                <div>
                  ...
                  <input type="password" id="password" name="password" value={formData.password} onChange={handleChange} className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700" required />
                </div>
                
                <div>
                  <label htmlFor="confirmPassword"className="block text-gray-700 font-bold mb-2">Confirmar Senha</label>
                  <input type="password" id="confirmPassword" name="confirmPassword" value={formData.confirmPassword} onChange={handleChange} className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700" required />
                </div>

                <button 
                    type="submit" 
                    className="w-full bg-uniguacu-red hover:bg-opacity-80 text-white font-bold py-3 px-4 rounded disabled:bg-gray-400"
