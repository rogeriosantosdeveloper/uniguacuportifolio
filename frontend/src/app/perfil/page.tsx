...
export default function PerfilPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Passo 3: Força a atualização dos dados no contexto para refletir as mudanças em toda a aplicação
      if (token) {
        await login(token);
      }
    ...
    <main className="container mx-auto p-8 max-w-2xl">
      ...
      >
        ...
        <div className="flex flex-col items-center gap-4">
          ...
          <div className="w-32 h-32 rounded-full border-4 border-uniguacu-blue bg-uniguacu-blue/10 flex items-center justify-center relative overflow-hidden">
            {previewImage ? (
              // Se existe uma imagem para preview (seja do servidor ou recém-selecionada)
              <img
                src={previewImage}
                alt="Foto de perfil"
              ...
              // Caso contrário (sem foto e sem seleção de arquivo), mostra a inicial
              user && (
                <span className="text-6xl font-bold text-uniguacu-blue">
        ...
        <div>
          <label htmlFor="curso" className="block text-gray-700 font-bold mb-2">
            Curso
          </label>
          <select
            id="curso"
        ...
        <div>
          <label htmlFor="turno" className="block text-gray-700 font-bold mb-2">
            Turno
          </label>
          <select
            id="turno"
