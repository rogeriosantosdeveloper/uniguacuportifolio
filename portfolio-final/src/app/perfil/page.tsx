...
export default function PerfilPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Passo 2: Atualiza os outros dados do usuário (nome, email, curso, turno)
      ...
      // Passo 3: Força a atualização dos dados no AuthContext para refletir em toda a aplicação
      if (token) {
          await login(token);
      }
  ...
  // Se os dados do usuário ainda não foram carregados do contexto
  if (!user) {
    return (
